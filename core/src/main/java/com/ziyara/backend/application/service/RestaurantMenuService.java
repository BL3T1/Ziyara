...
public class RestaurantMenuService implements RestaurantMenuApi {
    ...
    private final RestMenuSectionRepository sectionRepository;
    private final RestMenuItemRepository itemRepository;
    ...
    public RestaurantMenuResponse getMenu(UUID serviceId) {
        ...
        List<RestaurantMenuSectionResponse> sections = sectionRepository
                .findByServiceId(serviceId).stream()
    ...
    public RestaurantMenuSectionResponse updateSection(UUID serviceId, UUID sectionId, UpdateMenuSectionRequest request) {
        requireRestaurant(serviceId);
        RestMenuSection section = loadSectionOwnedByService(sectionId, serviceId);
        if (request.getTitle() != null) {
            String t = request.getTitle().trim();
            if (t.isEmpty()) throw new BusinessException("title cannot be empty");
            section.setTitle(t);
        }
    ...
    public void deleteSection(UUID serviceId, UUID sectionId) {
        requireRestaurant(serviceId);
        loadSectionOwnedByService(sectionId, serviceId);
    ...
    public RestaurantMenuItemResponse updateItem(UUID serviceId, UUID itemId, UpdateMenuItemRequest request) {
        requireRestaurant(serviceId);
        RestMenuItem item = loadItemOwnedByService(itemId, serviceId);
        if (request.getName() != null) {
            String n = request.getName().trim();
            if (n.isEmpty()) throw new BusinessException("name cannot be empty");
            item.setName(n);
        }
        if (request.getDescription() != null) item.setDescription(request.getDescription());
        if (request.getPrice() != null) item.setPrice(request.getPrice());
        if (request.getCurrency() != null) item.setCurrency(trimOrNull(request.getCurrency()));
        if (request.getImageUrl() != null) item.setImageUrl(trimOrNull(request.getImageUrl()));
    ...
    public void deleteItem(UUID serviceId, UUID itemId) {
        requireRestaurant(serviceId);
        loadItemOwnedByService(itemId, serviceId);
        itemRepository.deleteById(itemId);
    }

    @Transactional
    public RestaurantMenuItemResponse uploadItemImage(
            UUID serviceId, UUID itemId, byte[] fileBytes, String contentType, String originalFilename) {
        requireRestaurant(serviceId);
        RestMenuItem item = loadItemOwnedByService(itemId, serviceId);
    ...
    private RestMenuSection loadSectionOwnedByService(UUID sectionId, UUID serviceId) {
        RestMenuSection section = sectionRepository.findById(sectionId)
    ...
    private RestMenuItem loadItemOwnedByService(UUID itemId, UUID serviceId) {
        RestMenuItem item = itemRepository.findById(itemId)
                .orElseThrow(() -> new ResourceNotFoundException("Menu item not found"));
        RestMenuSection section = sectionRepository.findById(item.getSectionId())
                .orElseThrow(() -> new ResourceNotFoundException("Menu item not found"));
        if (!serviceId.equals(section.getServiceId())) {
    ...
    private RestaurantMenuSectionResponse toSectionResponse(RestMenuSection section) {
        List<RestaurantMenuItemResponse> items = itemRepository
                .findBySectionId(section.getId()).stream()
    ...
    private RestaurantMenuItemResponse toItemResponse(RestMenuItem item) {
