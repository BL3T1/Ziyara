-- Align legacy G{n} platform group codes with Z{n} (idempotent). Matches backend catalog / seed.
-- Update by canonical UUID only: a global G*→Z* UPDATE can hit unique(code) if Z rows already exist.
